/**
 * Otomatizon Clean PDF Document Generator (CommonJS & Client compatible)
...
function generateReportPdfBuffer(data) {
  ...
  const summaryText = (data.understood && data.understood.summary) || "Business operations audit";
  p1 += `BT /F1 10 Tf 0.071 0.075 0.086 rg 65 572 Td (${escapePdfText(summaryText.slice(0, 85))}) Tj ET\n`;
  p1 += `BT /F2 9 Tf 0.459 0.467 0.494 rg 65 550 Td (CLIENT TARGET SEGMENT:) Tj ET\n`;
  p1 += `BT /F1 9 Tf 0.071 0.075 0.086 rg 65 536 Td (${escapePdfText(data.understood && data.understood.customerType || "Individual learners")}) Tj ET\n`;
  p1 += `BT /F2 9 Tf 0.459 0.467 0.494 rg 65 518 Td (PRIMARY CHANNELS:) Tj ET\n`;
  const channels = (data.understood && data.understood.primaryChannels) || ["WhatsApp", "Referrals"];
  p1 += `BT /F1 9 Tf 0.071 0.075 0.086 rg 180 518 Td (${escapePdfText(channels.join(", "))}) Tj ET\n`;
  ...
  let yFlow = 425;
  const currentWf = data.currentWorkflow || [];
  currentWf.slice(0, 5).forEach((wf) => {
  ...
  let yOpp = 705;
  const opps = data.opportunitiesDiscovered || [];
  opps.slice(0, 2).forEach((opp) => {
    ...
    p2 += `BT /F1 8.5 Tf 0.459 0.467 0.494 rg 65 ${yOpp - 85} Td (Time Saved: ~${opp.estimatedTimeSavedHoursPerWeek} hrs/wk   |   Revenue Secured: KES ${(opp.estimatedRevenueAtRiskKes || 0).toLocaleString()}) Tj ET\n`;
    yOpp -= 135;
  });
  ...
  const firstAuto = data.recommendedFirstAutomation || { title: "Lead Follow-Up Autopilot", hoursSaved: 4.5 };
  p2 += `BT /F2 11 Tf 0.082 0.502 0.239 rg 65 368 Td (PRIORITY INITIATIVE: ${escapePdfText(firstAuto.title)}) Tj ET\n`;
  ...
  p2 += `BT /F2 9 Tf 0.082 0.502 0.239 rg 65 268 Td (Impact: Reclaims +${firstAuto.hoursSaved} hours/week directly.) Tj ET\n`;
  ...
  let yApps = 705;
  const appSummaries = data.requiredAppsSummary || [];
  appSummaries.slice(0, 5).forEach((app) => {
