// Otomatizon Funnel & Early Product Analytics
...
export interface EarlyProductMetrics {
  // Core Metric:
  activatedAutomationsPerPayingBusiness: number; // e.g. 1.8 automations / business
  // Secondary Metrics:
  timeToFirstAutomationMinutes: number; // e.g. 3.8 minutes
  opportunityActivationRatePercent: number; // e.g. 42%
  weeklyActiveBusinesses: number; // e.g. 8
  automationSuccessRatePercent: number; // e.g. 98.4%
  monthlyRecurringRevenueKes: number; // e.g. KES 4,990
  revenuePerBusinessKes: number; // e.g. KES 623
}
...
export function trackFunnelEvent(stage: FunnelStage, metadata?: Record<string, any>): FunnelEvent {
...
export function calculateEarlyProductMetrics(activeAutomationsCount: number = 1): EarlyProductMetrics {
