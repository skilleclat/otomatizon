 ...
 */
export function understandBusiness(ctx: BusinessContext): BusinessUnderstanding {
  const painPoints: string[] = [];
  let primaryBottleneck: OpportunityCategory = "lead_followup";
...
export function scoreOpportunity(
  businessImpactWeight: number, // 0.0 - 1.0
  frequencyWeight: number,     // 0.0 - 1.0
  confidenceWeight: number,    // 0.0 - 1.0
  feasibilityWeight: number    // 0.0 - 1.0
): OpportunityScoreResult {
 ...
 */
export function detectOpportunities(ctx: BusinessContext): OpportunityDecisionObject[] {
  const detectors = [
    detectLeadLeak(ctx),
    detectPaymentLeak(ctx),
    detectSchedulingFriction(ctx),
    detectRetentionGap(ctx),
    detectAdminRepetition(ctx)
  ];

  const results: OpportunityDecisionObject[] = [];
 ...
 */
const decisionAuditTrail: DecisionAuditEvent[] = [];

export function recordDecisionEvent(event: DecisionAuditEvent): void {
...
export function getDecisionAuditTrail(): DecisionAuditEvent[] {
...
export function calculateBusinessImpact(
  measuredRuns: number,
  averageDealKes: number = 3500
): {
  measured: MeasuredImpactMetrics;
  estimatedWeeklyHours: number;
  estimatedWeeklyRevenueKes: number;
} {
